package com.luck.library.widget;
...
/**
 * 自定义的 ViewGroup，具有监听手势滑动功能；
 * 用于封装 DecorView 中的LinearLayout；可使  Activity 具有滑动关闭效果；
 *
 */
public class GestureViewGroup extends ViewGroup {
	private Context context;
	private Scroller scroller;
	//标记该 ViewGroup 是否处于 fling 状态；
	private boolean isFling = false;
	private GestureDetector detector;
	private float lastX;
	private float lastY;
	private Handler mHandler = new GestureViewGroupHandler(this);

	private static class GestureViewGroupHandler extends Handler {
		private final WeakReference<GestureViewGroup> myGestureViewGroup;

		public GestureViewGroupHandler(GestureViewGroup gestureViewGroup) {
	...
	private void initview() {
		...
		detector = new GestureDetector(context, new GestureDetectorListener());
	}
	...
	private class GestureDetectorListener implements OnGestureListener {
		...
		public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
				float velocityY) {
			if (velocityX>100 && e2.getX()-e1.getX()>80 && Math.abs(velocityX)>Math.abs(velocityY) && Math.abs(e2.getX()-e1.getX())>Math.abs(e2.getY()-e1.getY())) {// 向右滑动;
				isFling = true;
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if(distanceX > distanceY && distanceX>10){//处理左右滑动
				result = true;
			}
